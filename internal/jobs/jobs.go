// Package jobs tracks in-flight OCR operations so the HTTP layer can stream
// their progress over SSE and support cancellation. State is in-memory only
// — this is a disposable local tool, not a service.
package jobs
...
type Event struct {
	Stage    string `json:"stage"` // ocr, done, error, canceled
...
func (r *Registry) Create(parent context.Context) (*Job, context.Context) {
	...
	// long sessions with many OCR runs.
...
// the idle-timeout shutdown so a long OCR run in progress (with no other
// page interaction) never gets killed out from under itself.
